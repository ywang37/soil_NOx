...
for iyr in range(start_year, end_year+1):
    ...
    for imo in month_list:
        ...
        TQ_file = root_dir + yyyy + '/MERRA2.' + yyyymm + '.' + res + '.nc'
        if coor_flag:
            coor_flag = False
            indata = read_nc(TQ_file, varnames=[varn1, varn2]+coor_var)
            lat   = indata['Latitude']
            lon   = indata['Longitude']
            lat_e = indata['Latitude_e']
            lon_e = indata['Longitude_e']
        else:
            indata = read_nc(TQ_file, varnames=[varn1, varn2])

        # temperature
        T_tmp = indata[varn1] - 273.15
        T_mon.append(T_tmp)
        ...
        Q_tmp = indata[varn2]
        Q_mon.append(Q_tmp)
        Q_monthly.append(Q_tmp)
        Q_month_dict[mm].append(Q_tmp)
    ...
    # moisture data
    Q_mon = np.array(Q_mon)
    Q_mon_mean = np.mean(Q_mon, axis=0)
    Q_year.append(Q_mon_mean)


# yearly
T_year = np.array(T_year)
Q_year = np.array(Q_year)

# mean
T_mean = np.mean(T_year, axis=0)
Q_mean = np.mean(Q_year, axis=0)

# monthly
T_monthly = np.array(T_monthly)
Q_monthly = np.array(Q_monthly)

# every month
for imo in month_list:
    mm = str(imo).zfill(2)
    T_month_dict[mm] = np.array(T_month_dict[mm])
    Q_month_dict[mm] = np.array(Q_month_dict[mm])
...
units_dict['mean_moisture'] = 'unitless'
...
# output
outfile = data_dir + 'corr_' + varn1 + '_' + varn2 + '_' + \
        sy_c + '-' + ey_c + '_winter' + '.nc'
