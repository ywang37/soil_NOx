...
soil_T_NO2_root_dir = '/Dedicated/jwang-data/ywang/soil_NOx/\
...
# variable name
varn = 'sat_ColumnAmountNO2Trop'

# NO2 variable name
#NO2_varn = 'sat_ColumnAmountNO2Trop'
NO2_varn_list = ['mod_NO2Trop_AK_tp_sat_ori',
...
soil_T_NO2_root_dir = soil_T_NO2_root_dir + \
        'soil_T_' + secne_dict[thre_scene] + \
        '_' + soil_T_thre + '/monthly/'
...
for NO2_varn in NO2_varn_list:
    ...
    for iyr in range(start_year, end_year+1):
        ...
        for imo in range(start_month, end_month+1):
            ...
            T_tmp = (indata[varn] / 1e15) # molec/cm2 => 1e15 molec/cm2
            T_mon.append(T_tmp)
            T_monthly.append(T_tmp)
            T_month_dict[mm].append(T_tmp)

            # read NO2 data
            NO2_file = soil_T_NO2_root_dir + \
                    thre_scene + '_' + soil_T_thre + \
                    '_model_satellite_' + \
                    yyyy + '-' + mm + '-01_' + \
                    yyyy + '-' + mm + '-' + month_days_dict[mm] + '.nc'
    ...
    # output
    outfile = data_dir + thre_scene + '_' + soil_T_thre + \
            '_corr_overpass_' + varn + '_' + NO2_varn + '_' + \
