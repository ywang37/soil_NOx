...
for month in month_list:
    ...
    fig_dir = fig_root_dir + month + '/'
    if not os.path.exists(fig_dir):
        os.system('mkdir -p ' + fig_dir)

    for ak in ak_list:

        sat_varname = 'sat_ColumnAmountNO2Trop_'+month
        ...
        for iyr in range(end_year - start_year + 1):

            year_data_dict = {}

            year_data_dict['Latitude_e'] = data_dict['Latitude_e']
            year_data_dict['Longitude_e'] = data_dict['Longitude_e']
            year_data_dict['OMI'] = data_dict['OMI'][iyr,:,:]
            year_data_dict['ori'] = data_dict['ori'][iyr,:,:]
            for i in range(len(scene_tup)):
                scene = scene_tup[i]
                mod_varname = mod_varname_list[i]
                year_data_dict['diff_'+scene] = \
                        data_dict['diff_'+scene][iyr,:,:]

            plot_compare_4_to_1_new(year_data_dict,
