...
def replace_run_sh(in_file, out_file,  year, month):
    ...
    data[1]  = data[1].replace('na_soil_NOx', 'm' + year[2:4] + month + \
            '_na_soil_NOx')
...
for year in year_list:
    for month in month_list:
        ...
        # copy run
        os.chdir(cwd_dir)
        run_dir = cwd_dir + '/tmp_na_sensitivity_2/' + YYYYMM
        if os.path.isdir(run_dir):
            os.system('rm -rf ' + run_dir)
        os.system('mkdir ' + run_dir)
        run_file_ori = './run_resample_na_sensitivity_2.sh'
        run_file = run_dir + \
                '/run_resample_na_sensitivity_2_' + YYYYMM  + '.sh'
