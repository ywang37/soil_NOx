...
for iyr in range(start_year, end_year+1):
    ...
    for imo in range(start_month, end_month+1):
        ...
        infile = in_root_dir + 'soil_T_' + secne_dict[thre_scene] + \
                '_' + soil_T_thre + \
                '/monthly/' + thre_scene + '_' + soil_T_thre + \
                '_model_satellite_' + \
                yyyy + '-' + mm + '-01_' + \
        ...
        if coor_flag:
            ...
            indata = read_nc(infile, varnames=varn_list, verbose=verbose)
    ...
    for varn in varn_list:
        multimonth_mean = np.nanmean(np.array(tmp_data[varn]), axis=0)
...
for varn in varn_list:
    ...
    for imo in range(start_month, end_month+1):
        ...
        all_var_data[varn]['month_'+mm+'_yearly_mean'] = \
                np.nanmean(all_var_data[varn]['month_'+mm], axis=0)

    # for multimonth
    all_var_data[varn]['multimonth_mean'] = \
            np.array(all_var_data[varn]['multimonth_mean'])
    all_var_data[varn]['multimonth_mean_yearly_mean'] = \
            np.nanmean(all_var_data[varn]['multimonth_mean'], axis=0)
...
# output
out_dir = out_root_dir + 'soil_T_' + secne_dict[thre_scene] + \
        '_' + soil_T_thre + '/multimonth_mean/'
if not os.path.exists(out_dir):
    os.system('mkdir -p ' + out_dir)
outfile = out_dir + thre_scene + '_' + soil_T_thre + '_model_satellite_' + \
