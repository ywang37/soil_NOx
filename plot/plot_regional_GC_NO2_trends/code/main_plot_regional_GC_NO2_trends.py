...
root_dir = '/Dedicated/jwang-data/ywang/soil_NOx/process/\
...
units = r'OMI L2 NO$_2$ [molec cm$^{-2}$]'
...
# variable name
varn = 'mod_NO2Trop_AK_tp_sat_' + experiment

# generate month
yyyymm_list = []
for year in range(start_year, end_year+1):
    for month in range(start_month, end_month+1):
        yyyy_mm_ = str(year) + '-' + str(month).zfill(2) + '-'
        end_date = month_days[str(month).zfill(2)]
...
for currMonth in yyyymm_list:
    ...
    T_file = root_dir + 'model_satellite_' + currMonth + '.nc'
...
fig_dir = fig_root_dir + experiment + '/'
...
for region in region_list:
    ...
    #
    sub_title = 'GC_NO2_' + experiment + '_' \
