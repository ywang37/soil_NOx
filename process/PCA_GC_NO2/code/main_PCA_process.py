...
inRootDir  = '/Dedicated/jwang-data/ywang/soil_NOx/process/\
...
experiment = 'soil_T_ori'
#experiment = 'ori'
#experiment = 'soil_T_obs'
#experiment = 'surf_T_obs'
...
varname = 'mod_NO2Trop_AK_tp_sat_' + experiment
...
for year in range(start_year, end_year+1):
    for month in month_list:
        yyyy_mm_ = str(year) + '-' + month + '-'
        end_date = month_days[month]
        yyyymm_list.append(yyyy_mm_ + '01_' + yyyy_mm_ + end_date)
...
for currMonth in yyyymm_list:
    ...
    # read data
    infile = inRootDir + 'model_satellite_' + currMonth + '.nc'
    if latlon_flag:
        latlon_flag = False
        in_data = read_nc(infile, [varname] + coord_name_list,
                verbose=True)
        for varn in coord_name_list:
            out_dict[varn] = in_data.pop(varn)
    else:
        in_data = read_nc(infile, [varname], verbose=True)
    in_var = in_data[varname]
...
out_file = outRootDir + 'PCA_' + str(start_year) + '-' + str(end_year) \
        + '_' + scene + '_' + experiment + '_' + land_ocean + '_' + res \
